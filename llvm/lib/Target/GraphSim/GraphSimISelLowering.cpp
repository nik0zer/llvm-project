...
#include "GraphSim.h"
#include "GraphSimMachineFunctionInfo.h"
...
#define DEBUG_TYPE "GraphSim-lower"
...
const char *GraphSimTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  case GraphSimISD::RET:
    return "GraphSimISD::RET";
  }
  return nullptr;
}
...
#include "GraphSimGenCallingConv.inc"
...
SDValue GraphSimTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_GraphSim);
  ...
  Chain = DAG.getNode(GraphSimISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_GraphSim);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
                                   ...
                                   const GraphSimSubtarget &Subtarget) {
...
static SDValue unpackFromRegLoc(SelectionDAG &DAG, SDValue Chain,
                                const CCValAssign &VA, const SDLoc &DL,
                                const GraphSimTargetLowering &TLI) {
  MachineFunction &MF = DAG.getMachineFunction();
...
/// GraphSim formal arguments implementation
SDValue GraphSimTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_GraphSim);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &GraphSim::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    GraphSimFunctionInfo *UFI = MF.getInfo<GraphSimFunctionInfo>();
...
bool GraphSimTargetLowering::CanLowerReturn(
...
SDValue
GraphSimTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_GraphSim);
  ...
  return DAG.getNode(GraphSimISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue GraphSimTargetLowering::PerformDAGCombine(SDNode *N,
...
bool GraphSimTargetLowering::isLegalAddressingMode(const DataLayout &DL,
